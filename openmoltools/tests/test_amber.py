...
def test_amber_binary_mixture():
    ...
    trj0, trj1 = md.load(water_filename), md.load(etoh_filename)

    # Hack to assign unique residue names that are consistent with contents of mol2 files
    trj0.top.residue(0).name = "WAT"
    trj1.top.residue(0).name = "ETH"
...
def test_amber_water_mixture():
    water_filename = utils.get_data_filename("chemicals/water/water-tripos.mol2")
    etoh_filename = utils.get_data_filename("chemicals/etoh/etoh-tripos.mol2")
    sustiva_filename = utils.get_data_filename("chemicals/sustiva/sustiva-tripos.mol2")
...
def test_run_antechamber():
    ...
    input_filename = utils.get_data_filename("chemicals/sustiva/sustiva-tripos.mol2")
    with utils.enter_temp_directory():  # Prevents creating tons of GAFF files everywhere.
