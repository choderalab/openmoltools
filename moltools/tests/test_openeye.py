...
def test_butanol_keepconfs():
    m0 = moltools.openeye.iupac_to_oemol("butanol")
    m1 = moltools.openeye.get_charges(m0, keep_confs=1)
...
def test_butanol():
    m0 = moltools.openeye.iupac_to_oemol("butanol")
    m1 = moltools.openeye.get_charges(m0)
    eq(m0.NumAtoms(), m1.NumAtoms())
    ...
    for k, molecule in enumerate(m1.GetConfs()):
        names_to_charges, str_repr = moltools.openeye.get_names_to_charges(molecule)
    ...
    with utils.enter_temp_directory():
        ...
        moltools.openeye.molecule_to_mol2(m1, "out.mol2")
        ...
        names_to_charges, str_repr = moltools.openeye.get_names_to_charges(m1)
...
@skipIf(not HAVE_OE, "Cannot test openeye module without OpenEye tools.")
def test_benzene():
    m0 = moltools.openeye.iupac_to_oemol("benzene")
    m1 = moltools.openeye.get_charges(m0)
    eq(m0.NumAtoms(), m1.NumAtoms())
    print(m1.NumConfs())
...
@skipIf(not HAVE_OE, "Cannot test openeye module without OpenEye tools.")
def test_smiles():
    m0 = moltools.openeye.smiles_to_oemol("CCCCO")
    charged0 = moltools.openeye.get_charges(m0)

    m1 = moltools.openeye.iupac_to_oemol("butanol")
    charged1 = moltools.openeye.get_charges(m1)
...
def test_ffxml():
    with utils.enter_temp_directory():    
        m0 = moltools.openeye.smiles_to_oemol("CCCCO")
        charged0 = moltools.openeye.get_charges(m0)
        m1 = moltools.openeye.smiles_to_oemol("ClC(Cl)(Cl)Cl")
        charged1 = moltools.openeye.get_charges(m1)

        trajectories, ffxml = moltools.openeye.oemols_to_ffxml([charged0, charged1])
...
def test_ffxml_simulation():
    ...
    with utils.enter_temp_directory():    
        m0 = moltools.openeye.smiles_to_oemol("Cc1ccccc1")
        charged0 = moltools.openeye.get_charges(m0)
        m1 = moltools.openeye.smiles_to_oemol("c1ccccc1")
        charged1 = moltools.openeye.get_charges(m1)
        ligands = [charged0, charged1]
        n_atoms = [15,12]

        trajectories, ffxml = moltools.openeye.oemols_to_ffxml(ligands)
...
def test_charge_fail1():
    with utils.enter_temp_directory():
        moltools.openeye.smiles_to_antechamber(smiles_fails_with_strictStereo, "test.mol2",  "test.frcmod", strictStereo=True)

@skipIf(not HAVE_OE, "Cannot test openeye module without OpenEye tools.")
@raises(RuntimeError)
def test_charge_fail2():
    m = moltools.openeye.smiles_to_oemol(smiles_fails_with_strictStereo)
    m = moltools.openeye.get_charges(m, strictStereo=True, keep_confs=1)

@skipIf(not HAVE_OE, "Cannot test openeye module without OpenEye tools.")
def test_charge_success1():
    with utils.enter_temp_directory():    
        moltools.openeye.smiles_to_antechamber(smiles_fails_with_strictStereo, "test.mol2",  "test.frcmod", strictStereo=False)

@skipIf(not HAVE_OE, "Cannot test openeye module without OpenEye tools.")
def test_charge_success2():
    m = moltools.openeye.smiles_to_oemol(smiles_fails_with_strictStereo)
    m = moltools.openeye.get_charges(m, strictStereo=False)
